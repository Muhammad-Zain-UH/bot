...
def write_result_txt(
    ...
    try:
        ...
        lines.append(f"  Generated : {datetime.now(PKT).strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        # SECTION 11: Footer
        lines.append("=" * 66)
        lines.append(f"Generated by XAUUSD Trading Bot | {datetime.now(PKT).strftime('%Y-%m-%d %H:%M:%S')}")
...
def write_signal_expired(reason: str, old_direction: str, old_conf: int) -> None:
  ...
  Generated: {datetime.now(PKT).strftime('%Y-%m-%d %H:%M:%S')}
================================================================
